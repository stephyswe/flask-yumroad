...
# If you're making live requests, this should be a real customer ID
DUMMY_CUSTOMER_ID = "cus_Ng84CNRBTFHcJ2"
# This email should correspond to the email on file for the customer ID above
TEST_CUSTOMER_EMAIL = "testuser@gmail.com"
...
def test_parse_webhook(app):
    app_webhook_secret = app.config.get(
        "STRIPE_WEBHOOK_KEY", DUMMY_WEBHOOK_SECRET
    )
    data, headers = mock_webhook(
        "checkout.session.completed", webhook_secret=app_webhook_secret
    )
...
def test_parse_invalid_webhook(app):
    data, headers = mock_webhook(
        "checkout.session.completed", webhook_secret="bad secret"
    )
...
@vcr.use_cassette(
    "tests/cassettes/test_get_customer.yaml",
...
def test_event_webhook(
    app, init_database, user_with_product, client, mail_outbox
):
    ...
    webhook_data, headers = mock_webhook(
        "checkout.session.completed", data=sample_data
    )
    response = client.post(
        url_for("checkout.stripe_webhook"),
        data=webhook_data,
        headers=headers,
...
def test_checkout_with_invalid_product_id(
    app, init_database, user_with_product, client
):
    product = user_with_product.store.products[0]
    sample_data = {
        "customer": DUMMY_CUSTOMER_ID,
