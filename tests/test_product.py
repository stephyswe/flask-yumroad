...
def create_book(
    name="Sherlock Homes", description="a house hunting real estate agent"
):
    book = Product(
        name="Sherlock Homes",
...
@vcr.use_cassette(
    "tests/cassettes/new_stripe_session.yaml",
...
def test_edit_submission(client, init_database):
    ...
    old_name = book.name
    response = client.post(
        url_for("product.edit", product_id=book.id),
...
def test_invalid_edit_submission(client, init_database):
    ...
    # Retrieve the book from the database using the stored book ID
    with client.application.app_context():
        assert (
            db.session.get(Product, session["book_id"]).description
            == old_description
        )
...
def test_delete_page(client, init_database):
    book = create_book()
    response = client.post(
        url_for("product.delete", product_id=book.id), follow_redirects=True
    )
    assert response.status_code == 200
...
def test_invalid_delete_page(client, init_database):
    book = create_book()
    response = client.post(
        url_for("product.delete", product_id=book.id + 1),
        follow_redirects=True,
    )
    assert response.status_code == 404
    assert (
        b"Whoops" in response.data
        and b"we couldn't find that product" in response.data
    )
