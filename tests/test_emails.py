...
from yumroad.email import (
    send_basic_welcome_message,
    send_welcome_message,
    send_pretty_welcome_message,
)
...
def test_template_email(
    app, init_database, mail_outbox, authenticated_request
):
...
def test_formatted_template_email(
    app, init_database, mail_outbox, authenticated_request
):
    user = User.query.first()
