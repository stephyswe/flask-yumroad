...
@product_bp.route(
    "/<int:product_id>/delete", methods=["GET", "POST", "DELETE"]
)
...
def details(product_id):
    ...
    print("Hello there product", product)
    print("Hello there SessionID", session_id)
    print("Hello there stripe_session", stripe_session)
    print("Hello there stripe_publishable_key", stripe_publishable_key)
...
def post_checkout(product_id):
    ...
    if purchase_state == "success" and post_purchase_session_id:
        flash(
            ...
            "You will receive an email shortly".format(product.name),
            "success",
        )
    elif purchase_state == "cancel" and post_purchase_session_id:
        flash(
            "There was an error while attempting to purchase this product. "
            "Please try again.",
            "danger",
        )
