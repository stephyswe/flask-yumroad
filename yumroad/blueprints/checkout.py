...
from yumroad.extensions import checkout, csrf
...
def stripe_webhook():
    # TODO: This line might raise an exception,
    # we need to handle it more elegantly
    event = checkout.parse_webhook(
        request.data.decode("utf-8"), request.headers
    )
    if event.type == "checkout.session.completed":
        print(event["data"])
        # If this logic becomes more complicated,
        # you may want to define functions for each event
